...
const Navbar = () => {
    ...
    // Get display role for admin (extract municipality from admin-bmc, admin-tmc, etc.)
    const getDisplayRole = () => {
        if (!role) return '';
        if (role === 'user') return 'USER';
        if (role.includes('admin')) {
            const municipality = role.replace('admin-', '').toUpperCase();
            return municipality === 'ADMIN' ? 'MAIN ADMIN' : municipality;
        }
    ...
    return (
        <nav className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center sticky top-0 z-[1000] shadow-sm font-sans">
            ...
            {/* Right Side - Dynamic based on page and login status */}
            <div className="flex items-center gap-4">
                ...
                {/* Logged In Pages (User/Admin Dashboards) - Show user profile and logout */}
                {isLoggedIn && !isLandingPage && !isAuthPage && (
                    <>
                        {/* User Profile Info */}
